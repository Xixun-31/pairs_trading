...
arb_train = get_price_history_range("arbitrum", start_days_ago=365, end_days_ago=200)
matic_train = get_price_history_range("matic-network", start_days_ago=365, end_days_ago=200)

# 合併成樣本內 DataFrame
data = pd.concat([
    arb_train.rename(columns={'price':'ARB-USD'}),
...
print(f"ARB & MATIC daily returns correlation: {correlation:.4f}")
...
print("\n=== 📊 ARB/MATIC Pairs Trading Performance ===")
...
plt.title("ARB/MATIC Pairs Trading Strategy Cumulative Return")
plt.xlabel("Date")
