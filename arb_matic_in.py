...
# 合併價格
data = pd.concat([
    arb.rename(columns={'price':'ARB-USD'}),
...
print(f"Overall daily returns correlation (ARB & MATIC): {overall_corr:.4f}")
...
positions.loc[zscore < -z_open, "ARB-USD"] = 1
...
plt.title("ARB/MATIC Pairs Trading Cumulative Return")
plt.xlabel("Date")
...
plt.title("Z-Score of ARB/MATIC Spread")
plt.xlabel("Date")
plt.ylabel("Z-Score")
plt.legend()
plt.grid(True)
